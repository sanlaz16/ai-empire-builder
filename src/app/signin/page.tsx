...
// Map Supabase error messages to friendly PT-BR equivalents
...
export default function SignIn() {
    ...
    const { signInWithGoogle, signInWithDev, user } = useAuth();
    ...
    useEffect(() => {
        // Dev bypass only in development builds — NOT on production
    ...
    const handleEmailLogin = async (e: React.FormEvent) => {
        ...
        // Explicit redirect on success — eliminates the infinite "Entrando..." bug
        router.push('/dashboard');
    };
    ...
    const handleForgotPassword = async () => {
        ...
        // Dynamic redirectTo works across local dev, Vercel preview, and production
        ...
        <div className="min-h-screen flex items-center justify-center relative overflow-hidden">
            ...
            <div className="container max-w-md relative z-10 px-4">
                <div className="card backdrop-blur-xl border-white/10 p-8 shadow-2xl">
                    ...
                    <form onSubmit={handleEmailLogin} className="space-y-4">
                        ...
                        <div>
                            <div className="flex justify-between items-center mb-1">
                                <label className="block text-sm font-medium text-gray-300">{t('auth.passwordLabel')}</label>
                                <button
                                    type="button"
                    ...
                    <button
                        onClick={async () => {
                            ...
                            if (error) setError(mapAuthError(error.message, t));
                        }}
                        type="button"
                    ...
                    {/* Dev bypass — only shown in development builds, NEVER in production */}
                    {isDev && (
                        <button
                ...
                <div className="text-center mt-8">
                    <Link href="/" className="text-gray-600 hover:text-gray-400 text-sm transition-colors">{t('auth.backToHome')}</Link>
                </div>
            </div>
        </div>
    );
